...
<style>
    ...
    /* oculta el toolbar flotante de Streamlit */
...
AUDIO_DEFAULTS = {
    ...
    "markers": [],           # lista de {time, label, note, color}
    "entities": None,        # dict con listas por tipo
    "lead_cache": None,      # titular + lead generado
}
...
def history_save_current():
    """Guarda el estado del audio activo en el historial."""
...
def history_load(audio_id):
    """Carga un audio del historial como activo."""
...
def reset_current_audio():
    """Limpia el audio activo sin tocar el historial."""
...
def retranscribe_gaps(client, audio_segment, gaps, model, status_writer=None):
    ...
    for gi, gap in enumerate(gaps):
        ...
        _, segments, _ = transcribe_single(client, gap_path, model, max_retries=3)
        if segments:
...
def transcribe_complete(client, path, model, progress_status=None):
    ...
    if progress_status:
        progress_status.write(f"📊 Cobertura: {coverage:.1f}%")
    ...
    if gaps and coverage < 98.0:
        ...
        if significant_gaps:
            ...
            recovered = retranscribe_gaps(client, audio_segment, significant_gaps, model, progress_status)
            if recovered:
...
# ============================================================
# BÚSQUEDA GLOBAL (entre todos los audios del historial)
# ============================================================

def global_search(query, audio_history, fuzzy_thresh=0.75):
    """Busca en todos los audios del historial."""
...
def extract_entities(client, text):
    """Extrae entidades nombradas del texto usando LLaMA."""
...
def highlight_entities_in_text(text, entities):
    """Aplica resaltado de entidades al texto."""
    if not entities or not text:
        return text

    # Construir lista de (término, clase) ordenada por longitud desc para evitar solapamiento
    pairs = []
...
def render_entity_panel(entities):
    """Renderiza el panel de entidades con colores."""
    if not entities:
        return
    cats = [
...
def generate_lead(client, text, filename=""):
    """Genera titular, subtítulo y lead en estilo periodístico."""
...
# ============================================================
# IA: ANÁLISIS (resumen, temas, acciones, tono)
# ============================================================
...
def main_app():
    ...
    if hist:
        ...
        # Botones de cambio de audio (uno por cada audio en historial)
        if len(hist) > 1:
    ...
    # ════════════════════════════════════════════════════
    # TAB: REDACCIÓN (vista dos paneles: audio+transcripción)
    # ════════════════════════════════════════════════════
    with tab_redaccion:
        # ── LEAD PERIODÍSTICO ──
        ...
        with panel_trans:
            ...
            # Toggle entre texto plano y segmentos
            ...
            # Opción de mostrar entidades en el texto
    ...
    with tab_global:
        if len(hist) <= 1:
            ...
            gq = st.text_input("gq", placeholder=f"Buscar en todos los audios ({len(hist)} archivos)...",
    ...
    with tab_analisis:
        ...
        if top_words:
            ...
            for word, freq in top_words:
                ...
                bars.append(
                    f"<div style='display:flex;align-items:center;gap:8px;margin:2px 0;font-size:0.76rem'>"
                    f"<span style='width:96px;font-weight:500;color:var(--text)'>{word}</span>"
                    f"<div style='flex:1;background:var(--bg);border-radius:4px;height:16px;overflow:hidden'>"
                    f"<div style='width:{pct}%;background:linear-gradient(90deg,#fed7aa,#ea580c);height:100%;border-radius:4px'></div>"
                    f"</div>"
                    f"<span style='width:28px;text-align:right;color:var(--text-secondary);font-size:0.7rem'>{freq}</span>"
                    f"</div>"
                )
    ...
    with tab_export:
        ...
        # Exportar sesión completa (todos los audios)
        if len(hist) > 1:
            st.markdown("---")
